...
import { ChevronRight, ArrowRight, BedDouble } from 'lucide-react';

export default function RoomsHero() {
  const { scrollYProgress } = useScroll();
  const y = useTransform(scrollYProgress, [0, 1], ['0%', '30%']);

  const headingText = "Rooms & Suites".split(" ");
  ...
  return (
    <section className="relative w-full h-[70vh] min-h-[500px] overflow-hidden">
      ...
      <div className="relative z-20 h-full max-w-[1280px] mx-auto px-6 lg:px-20 flex flex-col justify-center items-start pt-20">
        ...
        >
          Your personal sanctuary in the heart of Juba — where every room tells a story of comfort, elegance, and warm South Sudanese hospitality.
        </motion.p>
        ...
        >
          <button 
            onClick={() => document.getElementById('room-listings')?.scrollIntoView({ behavior: 'smooth' })}
          ...
          >
            <span className="relative z-10 group-hover:text-primary transition-colors duration-300">Explore Our Rooms</span>
        ...
        {/* Room Count Badge */}
        <motion.div 
          className="flex items-center gap-2 font-body text-[14px] text-white/60"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.5, delay: 1.4 }}
        >
          <BedDouble size={16} />
          50+ beautifully appointed rooms across 4 categories
        </motion.div>
